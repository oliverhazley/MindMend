...
export function initDashboard() {
  ...
  function groupByInterval(data, range) {
    ...
    const values = labels.map((label) => {
      const avg = buckets[label].reduce((a, b) => a + b, 0) / buckets[label].length;
  ...
  if (!labels.length) {
    ...
    elements.msgEl.textContent = "Not enough data, please keep tracking your progress.";
  ...
  setInterval(() => {
    ...
    if (elements.currentHRVValueEl && pulse) {
      ...
      elements.currentHRVTextEl.textContent = pulse < 60
    ...
    if (elements.rmssdValEl && rmssd) {
      ...
      elements.rmssdTextEl.textContent = rmssd < 40
